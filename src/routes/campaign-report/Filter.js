...
import { transformData, translateData } from 'services/campaign-report'
...
const Filter = ({
    ...
    const handleReset = () => {
        ...
        // fields.timezone = '8'
        fields.timezone = -(new Date().getTimezoneOffset()/60) + ''
        fields.platform = 'all'
        fields.adv_id = 'all'
    ...
    const handleExport = async () => {
        ...
        let data = translateData(fields);
        data.page_size = 10000000
        data.page_number = 1
        request({
            method: 'get',
            data: data,
            ...
            const data = [[
                ...
                'Advertising Spent',
                'Transaction record'
            ]].concat((res.result.items || []).map(record => transformData(record)).map(record => (

                [
                record.camp_id,
                record.camp_name,
                record.platformsForXlsx,
                record.impression,
                record.clicks,
                `${record.ctr} %`,
                moment(record.date).format('YYYY-MM-DD'),
                `${record.spent} DAD`,
                `https://explorer.dad.one/test/pages/address_en.html?${record.adv_account}`,
            ])))
            exportFile(data)
        })
    }
            ...
            */
            ...
            <Col {...ColProps} xl={{ span: 4 }} md={{ span: 8 }}>
                <FilterItem label="Time zone">
                    {getFieldDecorator('timezone', { initialValue: timezone ||  -(new Date().getTimezoneOffset()/60) + '' })(
